...
function RegionDropdowns({ type, onRegionChange, onUnitChange, onAddressChange, addressState }) {
    const [selectedRegion, setSelectedRegion] = useState(addressState.region);
    const [adminUnits, setAdminUnits] = useState([]);
    const [selectedAdminUnit, setSelectedAdminUnit] = useState(addressState.adminUnit);
    ...
    return (
        <Grid container spacing={3}>
            <Grid item xs={12}>
                <label>{type === "registration" ? "Registration Address" : "Actual Address"}</label>
            </Grid>

            <Grid item xs={12} sm={selectedAdminUnit ? 4 : adminUnits.length ? 6 : 12}>
                ...
                <Grid item xs={12} sm={selectedAdminUnit ? 4 : adminUnits.length ? 6 : 12}>
                    <FormControl fullWidth variant="outlined" className="new-input">
                        ...
                        <Select
                            ...
                            label={`${type.charAt(0).toUpperCase() + type.slice(1)} Administrative Unit`}
                        >
                            {adminUnits.map(unit => (
